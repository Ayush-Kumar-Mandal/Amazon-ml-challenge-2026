...
def tfidf_candidates(left: pl.DataFrame, right: pl.DataFrame, text_col: str, score_name: str,
    ...
    for country in groups:
        ...
        if L.height == 0 or R.height == 0:
            continue
        n = tcfg["ngram"]
        # max_df as a fixed *fraction* of the corpus lets the allowed per-trigram document
        # frequency grow with corpus size, which is what made blocking scale worse than
        # quadratically at full-country scale (see memory.md pitfalls). Cap it in absolute
        # terms too: effective_max_df = min(frac * corpus_size, max_df_abs). At small corpora
        # (dev) the fraction dominates and behavior is unchanged; at full scale the absolute
        # cap kicks in and bounds per-term cost independently of corpus size.
        corpus_size = L.height + R.height
        max_df_eff = min(max(int(tcfg["max_df_frac"] * corpus_size), tcfg["min_df"] + 1),
                         tcfg["max_df_abs"])
        vec = TfidfVectorizer(analyzer="char_wb", ngram_range=(n, n), min_df=tcfg["min_df"],
                              max_df=max_df_eff, sublinear_tf=True, dtype=np.float32)
        try:
